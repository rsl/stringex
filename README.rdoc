...
You can configure the default values to be used across all models by setting the class variables on Stringex::ActsAsUrl directly, using:
...
To run all the tests (including tests for ActiveRecord but not Mongoid)

  $ rake test

To run only the Mongoid tests

  $ rake test_mongoid

Note that currently 4 of the Mongoid tests fail. It is likely due to the setup function being run, which purges the database before each test. The code should be fine.
