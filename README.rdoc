...
You <em>may</em> however want to use Unidecoder to load localized codepoint transliterations. You can do so by loading either a Yaml file or Hash like the following:

  # Using a Hash
  LuckySneaks::Unidecoder::localize_from :en => {"é" => "ee"}

  # Loading from a file. NOTE: The path to file should be absolute.
  LuckySneaks::Unidecoder::localize_from "/path/to/yaml_file"
